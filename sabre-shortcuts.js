(function() {
    ...
    // Store initial PNR
    const initialBookingInfo = extractBookingInfo();
    let lastKnownPNR = initialBookingInfo.pnr;

    // Check for PNR changes every 2 seconds
    setInterval(function() {
        const currentInfo = extractBookingInfo();
        if (currentInfo.pnr && currentInfo.pnr !== lastKnownPNR) {
            console.log('PNR changed from', lastKnownPNR, 'to', currentInfo.pnr);
            // Remove old menu and reload
            const oldMenu = document.getElementById('sabreShortcutsMenu');
            if (oldMenu) oldMenu.remove();
            location.reload();
        }
    }, 2000);

    const bookingInfo = initialBookingInfo;
    let approvalHTML = '';
    ...
    menu.innerHTML = '<div class="menu-header">⚡ Sabre Shortcuts</div>' + bookingInfoHTML + 
        (bookingInfo.pnr || bookingInfo.traveller ? '<a href="#" class="menu-item" data-action="copyBookingInfo">Copy Booking Info</a>' : '') + notesButtonHTML +
    ...
    header.addEventListener('mousedown', function(e) {initialX = e.clientX - xOffset; initialY = e.clientY - yOffset; isDragging = true;});
    document.addEventListener('mousemove', function(e) {if (isDragging) {e.preventDefault(); currentX = e.clientX - initialX; currentY = e.clientY - initialY; xOffset = currentX; yOffset = currentY; menu.style.transform = 'translate3d(' + currentX + 'px, ' + currentY + 'px, 0)';}});
    document.addEventListener('mouseup', function() {isDragging = false;});
    menu.querySelector('.close-btn').addEventListener('click', function() {menu.remove();});
    ...
    function showNotesModal() {
        ...
        modal.querySelector('.notes-modal-close').addEventListener('click', function() {overlay.remove(); modal.remove();});
